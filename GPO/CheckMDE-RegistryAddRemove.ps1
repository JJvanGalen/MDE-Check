...
try {
    ...
    if ($currentDisplayName -ne $displayNameValue -or $currentDisplayVersion -ne $displayVersion) {
        ...
        Write-Log "Registry HKLM DisplayName/DisplayVersion ongewijzigd: $currentDisplayName"
    }
    if ($currentDisplayNameX86 -ne $displayNameValue -or $currentDisplayVersionX86 -ne $displayVersion) {
        ...
        Write-Log "Registry HKLM Wow6432Node DisplayName/DisplayVersion ongewijzigd: $currentDisplayName"
    }

    # Publisher/InstallDate alleen bij nieuwe versie (Publisher wijziging)
    ...
    if ($currentPublisher -ne $publisherValue) {
        ...
        Write-Log "Registry HKLM Publisher (-<version>) en InstallDate bijgewerkt: $publisherValue / $installDateToday"
    } else {
        Write-Log "Registry HKLM Publisher (-<version>) en InstallDate ongewijzigd: $currentPublisher / $currentInstallDate"
    }
    if ($currentPublisherX86 -ne $publisherValue) {
        ...
        Write-Log "Registry HKLM Wow6432Node Publisher (-<version>) en InstallDate bijgewerkt: $publisherValue / $installDateToday"
    } else {
        Write-Log "Registry HKLM Wow6432Node Publisher (-<version>) en InstallDate ongewijzigd: $currentPublisherX86 / $currentInstallDateX86"
    }
} catch {
